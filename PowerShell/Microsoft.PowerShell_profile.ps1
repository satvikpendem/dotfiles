...
function whisperv {
    ...
    # TODO: Replace official OpenAI Whisper with whisper-ctranslate2
    if ($Cpu) {
...
function yt {
    ...
    if ($Audio) {
        ...
        $output_format = "mp3"
        # $audio_format = "--audio-format $output_format"
        $extract_audio = "--extract-audio"
    }
    else {
        # $output_format = "mp4"
        # $merge_output_format = "--merge-output-format $output_format"
    ...
    yt-dlp --sponsorblock-remove all --progress --console-title --format $format $extract_audio --no-continue --embed-metadata --parse-metadata "%(title)s:%(meta_title)s" --parse-metadata "%(uploader)s:%(meta_artist)s" --embed-thumbnail --embed-subs --sub-langs "en.*,-live_chat" --concurrent-fragments 5 --output $output $merge_output_format "$Url"
