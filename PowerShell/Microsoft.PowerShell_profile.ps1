...
function yt {
    ...
    yt-dlp --sponsorblock-remove all --progress --console-title --format $format $extract_audio --no-continue --embed-metadata --parse-metadata "%(title)s:%(meta_title)s" --parse-metadata "%(uploader)s:%(meta_artist)s" --embed-thumbnail --embed-subs --sub-langs "all,-live_chat" --concurrent-fragments 5 --output $output $merge_output_format "$Url"
